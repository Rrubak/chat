<?php 
	...
	// print_r($result);
	 	if($result == "empty"){
	 		...
	 		echo '<li> <div class="message"> &#x1f618; User Sent You a Message Request .. Send to Start the Conversation &#x1f618;</div></li></ul></div>';
	 	}else{
			...
			$updated_status =str_replace($receiver_id[1],'0',$current_receiver_status[0]['status']);
			...
			// }
			// print_r($new_status);
			$result2 = update('`status`= "'.$updated_status.'"', 'users', '`id`= '.$_SESSION["user_details"]["userid"].'', $conn);
			// print_r($result2);
	 	}